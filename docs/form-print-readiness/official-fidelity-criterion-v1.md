...
2. **The tolerance metric is a displacement detector, not a fidelity measure.** It has 3–4 orders of magnitude of margin on translation, scale, ink-weight and deletion. It has *exactly zero* signal on content correctness: every statutory tax rate on 2551Q page 2 replaced with a wrong value scored a max per-region regression of `0.19e-4` and passed every existing assertion. That is not a tunable weakness; it is what the metric measures.
...
- **It does not prove the printed text is correct.** Wrong statutory tax rates on every ATC row scored `0.19e-4` — indistinguishable from zero — and passed every pixel component. Content correctness rests entirely on `static-text-exhaustive-v1`.
