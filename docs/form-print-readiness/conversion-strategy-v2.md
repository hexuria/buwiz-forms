...
### Stage 3 — Geometry-contract extraction (NEW: `scripts/extract_geometry_contract.py`, promoted from the spike scripts)
- **Command:** `rtk python3 scripts/extract_geometry_contract.py --pdf <pinned pdf> --expected-sha256 <pin> --out references/geometry-contracts/<CODE>-<REV>/`
- Emits `contract.json` (schema `bir-geometry-contract/draft-0`: merged/coalesced rules in pt and px, fill regions, full text runs with text/bbox/size/font/bold/italic, comb candidates from both detectors — tick-run v1 + container-aware v2 with smallest-container preference, checkbox candidates, image bboxes + xrefs, source sha256) plus per-page 144-DPI **overlay PNGs** for review. Coalescing tolerance auto-selected by the plateau sweep (0.5→3.0; 1602Q plateaus at 2.0).
- **Generated:** everything above. **Reviewed (mandatory):** comb/checkbox candidate accept/reject via overlays — the spike showed ~100% precision/recall on 1602Q p0/p1 ticks and ~28–29/29 on 2316 with 3–4 fixable false positives; this is *candidate generation*, not unsupervised detection. **Hand:** semantic naming (TIN vs ZIP) — always human.
- **Exit gate:** contract committed with accepted-candidate annotations; overlays archived as review evidence.
- **Time:** <1 s/page machine; overlay review minutes per page, ~15–45 min per form.
