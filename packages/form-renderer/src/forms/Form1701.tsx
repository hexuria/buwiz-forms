...
function ScheduleOne1701({ envelope }: { envelope: RenderEnvelope }) {
  return <section className="schedule-one-1701"><h3>Schedule 1 - Gross Compensation Income and Tax Withheld (Attach Additional Sheet/s, if necessary)</h3><p>On Items 1 and 2, enter the required information for each employer/s and mark (X) whether the information is for the Taxpayer or Spouse. On Item 3A, enter the Total Gross Compensation and Total Tax Withheld for the Taxpayer and on Item 3B, for the Spouse.</p><div className="employer-head-1701"><span>#</span><span>a. Name of Employer / b. Employer’s TIN</span></div>{[1, 2].map((index) => <div className="employer-row-1701" key={index}><span>{index}<CheckChoice checked={text(envelope, `employer_${index}_owner`) === "taxpayer"} label="Taxpayer" /><CheckChoice checked={text(envelope, `employer_${index}_owner`) === "spouse"} label="Spouse" /></span><span><AdaptiveCombValue value={text(envelope, `employer_${index}_name`)} cells={52} /><AdaptiveCombValue value={text(envelope, `employer_${index}_tin`)} cells={28} /></span></div>)}<div className="employer-continuation-head-1701"><span>(Continuation of Table Above)</span><span>c. Compensation Income</span><span>d. Tax Withheld</span></div>{[1, 2].map((index) => <div className="employer-amount-row-1701" key={index}><span>{index}</span><Amount1701 envelope={envelope} fieldKey={`employer_${index}_compensation`} /><Amount1701 envelope={envelope} fieldKey={`employer_${index}_withheld`} /></div>)}<div className="employer-total-1701"><span>3A Gross Compensation Income and Total Tax Withheld for TAXPAYER</span><Amount1701 envelope={envelope} fieldKey="schedule_2_4_taxpayer" /><Amount1701 envelope={envelope} fieldKey="part_vii_5_taxpayer" /></div><div className="employer-total-1701"><span>3B Gross Compensation Income and Total Tax Withheld for SPOUSE</span><Amount1701 envelope={envelope} fieldKey="schedule_2_4_spouse" /><Amount1701 envelope={envelope} fieldKey="part_vii_5_spouse" /></div></section>;
}
