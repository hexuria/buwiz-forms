...
impl Form2551QDraft {
    pub fn to_bir_xml_payload(&self) -> String {
        ...
        insert(&mut fields, "frm2551Qv2018:txtYear", self.taxable_year.to_string());
        for i in 1..=4 {
            insert_bool(&mut fields, &format!("frm2551Qv2018:qtr_{}", i), self.quarter == i);
        }
        ...
        insert(&mut fields, "frm2551Qv2018:txtBranchCode", branch.to_string());
        insert(&mut fields, "frm2551Qv2018:txtRDOCode", self.rdo_code.clone());
        insert(&mut fields, "frm2551Qv2018:registeredName", self.taxpayer_name.clone());
        insert(&mut fields, "frm2551Qv2018:registeredAddress", self.registered_address.clone());
        insert(&mut fields, "frm2551Qv2018:zipCode", self.zip_code.clone());
        insert(&mut fields, "frm2551Qv2018:telNo", self.contact_number.clone());
        insert(&mut fields, "txtEmail", self.email.clone());

        insert(&mut fields, "frm2551Qv2018:txt14", format!("{:.2}", self.total_tax_due));
        insert(&mut fields, "frm2551Qv2018:txt15", format!("{:.2}", self.creditable_tax_withheld));
        insert(&mut fields, "frm2551Qv2018:txt16", format!("{:.2}", self.tax_paid_previous));
        insert(&mut fields, "frm2551Qv2018:txt24", format!("{:.2}", self.tax_payable));
        insert(&mut fields, "txtTotalSched1", format!("{:.2}", self.total_tax_due));

        for i in 0..6 {
            let row = self.schedule_1.get(i);
            let atc = row.map(|r| r.atc.as_str()).unwrap_or("0");
            let atc_amt = row.map(|r| format!("{:.2}", r.taxable_amount)).unwrap_or("0.00".to_string());
            let atc_rate = row.map(|r| format!("{:.1}", r.tax_rate * 100.0)).unwrap_or("0.00".to_string());
            let atc_due = row.map(|r| format!("{:.2}", r.tax_due)).unwrap_or("0.00".to_string());
