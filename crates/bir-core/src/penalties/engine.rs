...
impl PenaltyEngine {
    pub fn calculate(ctx: &PenaltyContext, config: &PenaltyConfig) -> PenaltyResult {
        ...
        let is_micro_or_small = matches!(
            ctx.taxpayer_class,
            TaxpayerClass::Micro | TaxpayerClass::Small
        );

        // Multipliers and Rates from Config
        let surcharge_rate =
            config.get_surcharge_rate(ctx.taxpayer_class, ctx.is_fraud_or_willful_neglect);
        ...
        let surcharge = if unpaid_tax > 0.0 {
            unpaid_tax * surcharge_rate
        } else {
            0.0
        };
        let interest = if unpaid_tax > 0.0 {
            unpaid_tax * interest_rate * (days_late as f64 / 365.0)
        } else {
            0.0
        };
