...
impl TaxRule for ExclusiveGroupRule {
    fn name(&self) -> &'static str { "Exclusive Group Rule" }
    fn law(&self) -> &'static str { "BIR Form Instructions" }
    fn citation(&self) -> LegalCitation {
        LegalCitation { kind: CitationKind::BirFormInstruction, number: "1702".into(), section: "Annual Corporate ITR variants".into(), year: 2018 }
    }
    ...
    fn evaluate(&self, profile: &TaxpayerProfile, form: &TemporalFormDef, current_state: FormEligibility, _target_year: u16) -> FormEligibility {
        ...
        if group == "ANNUAL_CORPORATE_ITR" {
            let winner = match &profile.tax_classification {
                Some(TaxClassification::CooperativeExempt) => "1702EX",
                Some(TaxClassification::CooperativeMixed) => "1702MX",
                Some(TaxClassification::Corporation) | Some(TaxClassification::CooperativeTaxable) => "1702RT",
                _ => "1702RT", // Default for corporations
            };
            if form.code == winner {
                return current_state; // Keep this one
            } else {
                return FormEligibility::Suppressed(format!("Exclusive group: {} selected instead", winner));
            }
        }
        current_state
    }
